...
class MetadataTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.mock_metadata2 = {
            ...
            'programmatic_descriptions': [
                {'source': 'c_1', 'text': 'description c', 'is_editable': 'false'},
                {'source': 'a_1', 'text': 'description a', 'is_editable': 'false'},
                {'source': 'b_1', 'text': 'description b', 'is_editable': 'false'}
            ]
        }
    ...
    def test_get_table_programmatic_metadata_with_config_success(self) -> None:
        ...
        local_app.config['PROGRAMMATIC_DISPLAY'] = {
            "a_1": {"display_order": 0, "title": "S3"},
            "b_1": {"display_order": 1, "title": "Quality"},
            "c_1": {"display_order": 2, "title": "Nobody Cares"}
        }
        url = local_app.config['METADATASERVICE_BASE'] + TABLE_ENDPOINT + '/db://cluster.schema/table'
        responses.add(responses.GET, url, json=self.mock_metadata2, status=HTTPStatus.OK)
