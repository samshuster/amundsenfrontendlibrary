...
def _update_prog_descriptions(prog_descriptions: List) -> None:
    ...
    if app.config['PROGRAMMATIC_DISPLAY'] and prog_descriptions:
        ...
        for desc in prog_descriptions:
            ...
            if not desc_source:
                continue
            config_dict = prog_display_config.get(desc_source)
            # But people might not have specified a configuration for all sources...
            if config_dict:
                desc['title'] = config_dict['title']
